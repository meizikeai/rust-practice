...
use crate::{
    config,
    model::domain::{CacheClient, DbClient, DbManager},
};
...
impl Connect {
    ...
    pub async fn new(config: config::Config) -> (DbClient, CacheClient) {
        ...
        (DbClient { relation: DbManager { master: relation_master, slave: relation_slave } }, CacheClient { profile })
    }
}
